...
void MF_NutHandlerKeyF8 (void *handle)		// Handler to display release infos
{
	...
	strcat(buf,"MailFilter/ax professional Server/NLM ");
	strcat(buf,MAILFILTERVER_MAJOR);
	strcat(buf,"/");
...
int main( int argc, char *argv[ ])
{
	...
	consoleprintf("MAILFLT: Info: This is the Legacy version of MailFilter/ax!\n  Use only on NetWare 5.0, NetWare 5.1 prior SP6 or NetWare 6.0 prior SP3.\n");
#else
	{
		struct utsname u;
		uname(&u);
...
//	SetTargetNameSpace(LONGNameSpace);
	...
	if (MFT_Verbose)
	{
		...
		if (OpenScreen ( "MailFilter/ax Debug", MFD_ScreenTag, &MFD_ScreenID))
		{
