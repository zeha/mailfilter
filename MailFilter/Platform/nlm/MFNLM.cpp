...
void MF_NutHandlerKeyF6 (void *handle)		// Handler F6 = ShowDetails()
{
	handle = handle;			// Keep compiler quiet.
...
/*	sprintf(buf,MF_Msg(LOGMSG_STATISTICS_LINE1));
	MF_StatusUI_UpdateLog(buf);

	sprintf(buf,MF_Msg(LOGMSG_STATISTICS_LINE2),MFS_MF_MailsInputTotal,MFS_MF_MailsInputFailed);
	MF_StatusUI_UpdateLog(buf);

	sprintf(buf,MF_Msg(LOGMSG_STATISTICS_LINE3),MFS_MF_MailsOutputTotal,MFS_MF_MailsOutputFailed);
	MF_StatusUI_UpdateLog(buf);

	unsigned long clck = clock() / CLOCKS_PER_SEC / 60;
	sprintf(buf,MF_Msg(LOGMSG_STATISTICS_LINE4),(clck/60) , (clck) - (((long)(clck/60))*60));
	MF_StatusUI_UpdateLog(buf);*/
	...
	MFBlacklist bl("bl.spamcop.net","127.0.0.2");	// = new MFBlacklist
