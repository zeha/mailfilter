...
struct MimeDecoderData {
  ...
  int (*write_buffer) (const char *buf, int size, void *closure);
  void *closure;
};


static int
mime_decode_qp_buffer (MimeDecoderData *data, const char *buffer, int length)
{
	  ...
	  if (token [0] == '=')
		{
		  ...
		  if (token[1] >= '0' && token[1] <= '9')
			...
			c = token[1] - ('a' - 10);
		  else if (token[1] == IX_CR || token[1] == IX_LF)
			{
			  /* =\n means ignore the newline. */
			  if (token[1] == IX_CR && token[2] == IX_LF)
...
mime_decode_uue_buffer (MimeDecoderData *data,
						const char *input_buffer, int input_length)
{
...
mime_decoder_init (mime_encoding which,
				   int (*output_fn) (const char *, int, void *),
				   void *closure)
{
  MimeDecoderData *data = new(MimeDecoderData);
  if (!data) return 0;
...
MimeDecoderData *
MimeB64DecoderInit (int (*output_fn) (const char *, int, void *),
					void *closure)
{
...
struct MimeEncoderData {
  ...
  int current_column, line_byte_count;
...
mime_uuencode_write_line(MimeEncoderData *data)
{
	...
	data->uue_line_buf[data->current_column++] = IX_CR;
	data->uue_line_buf[data->current_column++] = IX_LF;
...
int
mime_encode_qp_buffer (MimeEncoderData *data, const char *buffer, int size)
{
  int status = 0;
...
//			 data->in_buffer_count == 0, "1.1 <rhp@netscape.com> 19 Mar 1999 12:00");
  ...
  if (!abort_p &&
	  data->in_buffer_count > 0)
	{
	   ...
	   */
	  unsigned int n = ((unsigned int) data->in_buffer[0]) << 16;
	  if (data->in_buffer_count > 1)
		n = n | (((unsigned int) data->in_buffer[1]) << 8);
  ...
  if (data->filename != NULL) free (data->filename);
...
mime_encoder_init (mime_encoding which,
				   ...
				   void *closure)
{
  MimeEncoderData *data = new(MimeEncoderData);
  if (!data) return 0;
  memset(data, 0, sizeof(*data));
  data->encoding = which;
  data->write_buffer = output_fn;
...
MimeEncoderData *
MimeB64EncoderInit (int (*output_fn) (const char *, int, void *),
					void *closure)
{
