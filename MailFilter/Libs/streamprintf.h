...
#include <stdio.h>
#include <screen.h>
#define assertmsg(exp,msg)	if (!(exp)) consoleprintf( "MAILFILTER ASSERT: %s\n",msg)
#endif

#ifdef IXPLAT_NETWARE_CLIB
#define assertmsg(exp,msg)	if (!(exp)) MF_DisplayCriticalError( "MAILFILTER ASSERT: %s\n",msg)
#endif
...
protected:
	...
	void my_vsprintf(CharT* output, const CharT* format, va_list vl);
...
template <>
inline void Printf<char>::my_vsprintf(char* output, const char* format, va_list vl)
{
	vsprintf(output, format, vl);
}

//template <>
//inline void Printf<wchar_t>::my_vsprintf(wchar_t* output, const wchar_t* format, va_list vl)
//{
...
void Printf<CharT>::Do(int sizeAndType, ...)
{
	...
	else if (type == Unsigned && size == Short)
	{
		...
		if (fmtChar == 'c')	// wchar_t is intended, so width must be 'l'
			assertmsg(sizeChar == 'l',"printf: Type mismatch c->l");
		else				// unsigned short is intended, so width must be 'h'
			assertmsg(sizeChar == 'h', "printf: Type mismatch c->h");
	}
	else
	{
		switch (size)
		...
		case None:
			assertmsg(sizeChar == '\0', "printf: Type mismatch (0)");
			break;
		case Long:
			assertmsg(sizeChar == 'l', "printf: Type mismatch (l)");
			break;
#else
		case None:
		case Long:
			assert(sizeof(int) == sizeof(long));
			assertmsg(sizeChar == '\0' || sizeChar == 'l', "printf: Type mismatch (l0)");
			break;
#endif
		case Short:
			assertmsg(sizeChar == 'h', "printf: Type mismatch (h)");
			break;
		case Int64:
			assertmsg(sizeChar == 'I', "printf: Type mismatch (I)");
			break;
		}
	}
